...
import { Plus, Edit2, Trash2, Calendar, Activity, Tag, Info } from 'lucide-react'
...
export function GestaoAnimais() {
  ...
  const handleSave = async () => {
    ...
    if (user) {
      ...
      let error
      if (formData.id) {
        const res = await supabase.from('animais').update(payload).eq('id', formData.id)
        error = res.error
      } else {
        const res = await supabase.from('animais').insert({ ...payload, user_id: user.id })
        error = res.error
      }
  ...
  const getProximoCio = (tipo: string, ultimaData: string) => {
    ...
    // Ciclo estral médio de 21 dias como base
