import { useState } from 'react'
import { Link } from 'react-router-dom'
import { Menu, X } from 'lucide-react'
import { Button } from '@/components/ui/button'

export function Header() {
  const [isMenuOpen, setIsMenuOpen] = useState(false)

  const toggleMenu = () => setIsMenuOpen(!isMenuOpen)

  return (
    <header className="navbar-glass">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex h-20 items-center justify-between">
          <div className="flex items-center">
            <Link to="/" className="flex items-center transition-opacity hover:opacity-90 group">
              <span className="text-2xl font-black tracking-tight text-agro-green drop-shadow-[0_0_15px_rgba(34,197,94,0.5)] group-hover:scale-105 transition-transform duration-400">
                AgroIA
              </span>
            </Link>
          </div>

          {/* Desktop Navigation */}
          <nav className="hidden md:flex items-center gap-8">
            <a
              href="#features"
              className="text-sm font-bold text-white/80 hover:text-[#8B5CF6] hover:drop-shadow-[0_0_10px_rgba(139,92,246,0.8)] transition-all duration-400"
            >
              Funcionalidades
            </a>
            <a
              href="#planos"
              className="text-sm font-bold text-white/80 hover:text-[#8B5CF6] hover:drop-shadow-[0_0_10px_rgba(139,92,246,0.8)] transition-all duration-400"
            >
              Planos
            </a>
            <Link
              to="/login"
              className="text-sm font-bold text-white/80 hover:text-white transition-all duration-400"
            >
              Login
            </Link>
            <Link to="/cadastro" className="btn-agro-primary px-6 py-2">
              Começar Agora
            </Link>
          </nav>

          {/* Mobile Menu Button */}
          <div className="flex items-center md:hidden">
            <Button
              variant="ghost"
              size="icon"
              onClick={toggleMenu}
              aria-label="Toggle menu"
              className="text-white hover:bg-white/20 hover:text-white"
            >
              {isMenuOpen ? <X className="h-6 w-6" /> : <Menu className="h-6 w-6" />}
            </Button>
          </div>
        </div>
      </div>

      {/* Mobile Navigation */}
      {isMenuOpen && (
        <div className="md:hidden absolute top-full left-0 w-full bg-[#111827]/95 backdrop-blur-xl shadow-lg animate-in slide-in-from-top-2 border-t border-white/10">
          <div className="flex flex-col space-y-2 px-4 pb-6 pt-4">
            <a
              href="#features"
              className="px-4 py-3 text-base font-bold text-white/80 hover:bg-white/5 hover:text-[#8B5CF6] rounded-lg transition-all duration-400"
              onClick={toggleMenu}
            >
              Funcionalidades
            </a>
            <a
              href="#planos"
              className="px-4 py-3 text-base font-bold text-white/80 hover:bg-white/5 hover:text-[#8B5CF6] rounded-lg transition-all duration-400"
              onClick={toggleMenu}
            >
              Planos
            </a>
            <Link
              to="/login"
              className="px-4 py-3 text-base font-bold text-white/80 hover:bg-white/5 hover:text-[#8B5CF6] rounded-lg transition-all duration-400"
              onClick={toggleMenu}
            >
              Login
            </Link>
            <div className="px-4 pt-4 pb-2">
              <Link
                to="/cadastro"
                className="btn-agro-primary w-full py-4 text-center block text-lg"
              >
                Começar Agora
              </Link>
            </div>
          </div>
        </div>
      )}
    </header>
  )
}
