...
function calcularBalancoHidrico(dados: any, cultura: string) {
  ...
  const chuva_7d = (dados.daily?.precipitation_sum ?? []).reduce(
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Previsão 7 dias
    const previsao7d = (dados.daily?.et0_fao_evapotranspiration ?? [])
      .slice(3)
      .map((et0: number, i: number) => ({
        dia: i + 1,
      ...
      JSON.stringify({
        ...
        data: {
          decisao: {
            ...
            melhor_horario:
              temperatura > 28 ? 'Antes das 9h ou após as 17h' : 'Qualquer horário nublado',
          },
