...
const PRECOS_FALLBACK: Record<string, number> = {
  'Soja': 135.50, 'Milho': 62.00, 'Trigo': 84.00,
  'Café': 1450.00, 'Cana-de-açúcar': 145.50, 'Algodão': 105.00, 'Boi Gordo': 285.50
};
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: dentroDoLimite } = await supabase.rpc('check_rate_limit', {
      p_user_id: rateLimitKey, p_function: 'gemini-forecast', p_max_requests: 20, p_window_minutes: 10
    });
    if (dentroDoLimite === false) throw new Error('Limite de previsões atingido. Tente novamente em alguns minutos.');
