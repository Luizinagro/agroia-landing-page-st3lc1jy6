...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === 'listar_insumos') {
      ...
      const abaixo_minimo = (data || []).filter(
        (i: any) => Number(i.estoque_atual) <= Number(i.estoque_minimo),
      )
      const valor_total_estoque = (data || []).reduce(
        (s: number, i: any) => s + Number(i.estoque_atual) * Number(i.preco_unitario),
        0,
      )
      return ok({
        insumos: data,
        alertas_estoque: abaixo_minimo.length,
        valor_total_estoque: Number(valor_total_estoque.toFixed(2)),
      })
    }
    ...
    if (action === 'entrada_estoque') {
      const { insumo_id, quantidade, data, fornecedor, nota_fiscal, preco_unitario, observacao } =
        body
    ...
    if (action === 'saida_estoque') {
      const { insumo_id, quantidade, data, talhao, cultura, safra, tipo_aplicacao, observacao } =
        body
      if (!insumo_id || !quantidade) return err('"insumo_id" e "quantidade" são obrigatórios')
    ...
    if (action === 'historico_movimentacoes') {
      ...
      let q = admin
        .from('insumos_movimentacoes')
        .select(`*, insumos_cadastro(nome, unidade, categoria)`)
    ...
    if (action === 'dashboard_insumos') {
      ...
      const abaixo_minimo = (insumos || []).filter(
        (i: any) => Number(i.estoque_atual) <= Number(i.estoque_minimo),
      )
      ...
      return ok({
        ...
        produtos_sem_estoque: (insumos || []).filter((i: any) => Number(i.estoque_atual) <= 0)
          .length,
        por_categoria: categorias,
        produtos_criticos: abaixo_minimo.map((i: any) => ({
          id: i.id,
          nome: i.nome,
    ...
    return err(
      `Ação desconhecida: "${action}". Disponíveis: criar_insumo, listar_insumos, atualizar_insumo, entrada_estoque, saida_estoque, historico_movimentacoes, dashboard_insumos`,
    )
  } catch (error: any) {
