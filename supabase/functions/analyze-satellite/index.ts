  ...
  try {
    ...
    if (sentinelClientId && sentinelClientSecret) {
      try {
        ...
        throw new Error('As chaves estão configuradas mas a infraestrutura da API não respondeu. Usando fallback de simulação.');
      } catch (e) {
        isMock = true;
        console.warn("Falha na integração com a API do Sentinel Hub, ativando fallback avançado.", e);
      }
    } else {
      isMock = true;
    }
    ...
    return new Response(JSON.stringify({
      ...
      message: isMock ? "Dados de demonstração - Conexão Sentinel Hub em modo Sandbox" : "Dados processados em tempo real pela API de Satélite."
    }), {
