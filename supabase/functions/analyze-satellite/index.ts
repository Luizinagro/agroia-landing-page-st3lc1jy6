...
function estimarNDVI(weather: any): number {
  ...
  const chuva7dias = (weather.daily?.precipitation_sum ?? []).reduce(
  ...
  const ndvi = 0.15 + (balancioHidrico * 0.3 + fatorTemp * 0.25 + fatorUmidade * 0.3)
  return Number(Math.min(0.92, Math.max(0.1, ndvi)).toFixed(2))
}
      ...
      body: JSON.stringify({
        input: {
          ...
          data: [
            {
              type: 'sentinel-2-l2a',
              dataFilter: { timeRange: { from: dataInicio, to: dataFim }, maxCloudCoverage: 30 },
            },
          ],
        },
        aggregation: {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (geminiKey) {
      const statusNDVI =
        ndvi_value > 0.65 ? 'SAUDÁVEL ✅' : ndvi_value > 0.45 ? 'ATENÇÃO ⚠️' : 'CRÍTICO 🔴'
