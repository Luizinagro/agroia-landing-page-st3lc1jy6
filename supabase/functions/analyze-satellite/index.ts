...
function estimarNDVI(weather: any): number {
  ...
  const fatorTemp = temp >= 15 && temp <= 38 ? 1 - Math.abs(temp - 25) / 30 : 0.3;
...
async function buscarNDVISentinel(lat: number, lon: number): Promise<number | null> {
  ...
  try {
    ...
    const statsRes = await fetch('https://services.sentinel-hub.com/api/v1/statistics', {
      ...
      body: JSON.stringify({
        input: { bounds: { bbox, properties: { crs: 'http://www.opengis.net/def/crs/EPSG/0/4326' } },
          data: [{ type: 'sentinel-2-l2a', dataFilter: { timeRange: { from: dataInicio, to: dataFim }, maxCloudCoverage: 30 } }] },
        aggregation: { timeRange: { from: dataInicio, to: dataFim }, aggregationInterval: { of: 'P30D' },
          evalscript: `//VERSION=3\nfunction setup(){return{input:[{bands:["B04","B08"]}],output:[{id:"ndvi",bands:1}]};}\nfunction evaluatePixel(s){const n=(s.B08-s.B04)/(s.B08+s.B04);return{ndvi:[isNaN(n)?0:n]};}`,
          resx: 10, resy: 10 }
      })
    });
    ...
    console.warn('Sentinel Hub indisponível:', e);
    return null;
  }
}
...
function gerarImagemSatelite(lat: number, lon: number): string {
  const delta = 0.02;
  const bbox = `${lon - delta},${lat - delta},${lon + delta},${lat + delta}`;
  return `https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/export?bbox=${bbox}&bboxSR=4326&size=600,400&imageSR=4326&format=jpg&transparent=false&f=image`;
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (latitude === undefined || longitude === undefined) throw new Error('Latitude e Longitude são obrigatórios.');
    ...
    const chuva7dias = Number(((weatherData.daily?.precipitation_sum ?? []).reduce((a: number, b: number) => a + (b ?? 0), 0)).toFixed(1));

    let ai_analysis = '';
    if (geminiKey) {
      const statusNDVI = ndvi_value > 0.65 ? 'SAUDÁVEL ✅' : ndvi_value > 0.45 ? 'ATENÇÃO ⚠️' : 'CRÍTICO 🔴';
      const prompt = `Você é um agrônomo especialista em agricultura tropical brasileira. Analise os dados REAIS abaixo de uma propriedade rural e responda em português de forma clara e objetiva.\n\n📍 Localização: Lat ${lat.toFixed(4)}, Lon ${lon.toFixed(4)}\n🌿 NDVI: ${ndvi_value} — ${statusNDVI}\n🌡️ Temperatura: ${temperature}°C\n💧 Umidade do solo: ${soil_moisture}%\n💦 Umidade do ar: ${humidity}%\n🌧️ Chuva hoje: ${precipitation}mm\n🌧️ Chuva 7 dias: ${chuva7dias}mm\n📡 Fonte: ${fonte_dados}\n\nResponda com:\n1) **Diagnóstico** (2 frases)\n2) **Recomendações** (máximo 3 ações práticas)\n3) **Alertas** (riscos de seca, pragas, etc.)`;
      try {
    ...
    const { data: inserted, error: dbError } = await supabaseAdmin
      ...
      .insert({ user_id: user.id, latitude, longitude, ndvi_value, soil_moisture, temperature, image_url, analysis_date })
      .select().single();
    if (dbError) console.error('Erro ao salvar:', dbError);

    const resultado = inserted ?? { id: crypto.randomUUID(), ndvi_value, soil_moisture, temperature, image_url, analysis_date };
    return new Response(JSON.stringify({
      success: true,
      data: { ...resultado, humidity, precipitation, chuva7dias, fonte_dados, ai_analysis },
      message: `✅ Análise com dados reais — ${fonte_dados}`
