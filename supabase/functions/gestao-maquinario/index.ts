...
async function initTables(admin: any) {
  ...
  for (const stmt of statements) {
    await admin
      ...
      .catch(() => null)
    try {
      await fetch(`${Deno.env.get('SUPABASE_URL')}/rest/v1/rpc/exec_sql`, {
        method: 'POST',
        headers: {
  ...
  try {
    ...
    if (action === 'listar_maquinas') {
      const { data, error } = await admin
        .from('maquinas')
        .select(
          `*, maquinas_documentos(tipo, vencimento), maquinas_manutencao(descricao, horas_gatilho, data_gatilho, status)`,
        )
        .eq('user_id', user.id)
      ...
      const result = (data || []).map((m: any) => {
        ...
        const manut_pendentes = (m.maquinas_manutencao || []).filter(
          (mn: any) => mn.status === 'agendado',
        )
        return {
          ...m,
          alertas: {
    ...
    if (action === 'registrar_horimetro') {
      ...
      await admin
        .from('maquinas')
        .update({ horimetro_atual: horas, updated_at: new Date().toISOString() })
        .eq('id', maquina_id)
        .eq('user_id', user.id)
      return ok(d)
    }
    ...
    if (action === 'adicionar_despesa') {
      ...
      if (!maquina_id || !categoria || !valor)
        return err('"maquina_id", "categoria" e "valor" são obrigatórios')
      const { data: d, error } = await admin
        .from('maquinas_despesas')
        .insert({
          maquina_id,
          categoria,
          descricao,
    ...
    if (action === 'criar_manutencao') {
      const { maquina_id, descricao, tipo_gatilho, horas_gatilho, data_gatilho, intervalo_horas } =
        body
      if (!maquina_id || !descricao) return err('"maquina_id" e "descricao" são obrigatórios')
      const { data, error } = await admin
        .from('maquinas_manutencao')
        .insert({
          maquina_id,
          descricao,
          tipo_gatilho: tipo_gatilho || 'data',
          horas_gatilho,
          data_gatilho,
          intervalo_horas,
          status: 'agendado',
    ...
    if (action === 'criar_documento') {
      const { maquina_id, tipo, descricao, vencimento, valor_seguro, seguradora, numero_apolice } =
        body
      if (!maquina_id || !tipo) return err('"maquina_id" e "tipo" são obrigatórios')
      const { data, error } = await admin
        .from('maquinas_documentos')
        .insert({
          maquina_id,
          tipo,
          descricao,
    ...
    if (action === 'resumo_frota') {
      ...
      const { data: maquinas } = await admin
        .from('maquinas')
        .select('id, nome, tipo, horimetro_atual')
        .eq('user_id', user.id)
    ...
    if (action === 'alertas') {
      ...
      for (const m of maquinas || []) {
        const { data: docs } = await admin
          .from('maquinas_documentos')
          .select('tipo, descricao, vencimento')
          .eq('maquina_id', m.id)
        for (const d of docs || []) {
        ...
        const { data: manuts } = await admin
          .from('maquinas_manutencao')
          .select('descricao, data_gatilho, horas_gatilho, status')
          .eq('maquina_id', m.id)
          .eq('status', 'agendado')
        ...
        for (const mn of manuts || []) {
          if (mn.data_gatilho) {
            const diff = Math.ceil(
              (new Date(mn.data_gatilho).getTime() - hoje.getTime()) / 86400000,
            )
            if (diff <= 14)
              alertas.push({
                maquina: m.nome,
                ...
                dias_restantes: diff,
                urgencia: diff < 0 ? 'atrasado' : 'atencao',
              })
          }
          if (mn.horas_gatilho && maq) {
            ...
            if (horas_restantes <= 50)
              alertas.push({
                maquina: m.nome,
                tipo: 'manutencao_horas',
                descricao: mn.descricao,
                horas_restantes: Math.round(horas_restantes),
                urgencia: horas_restantes <= 0 ? 'atrasado' : 'atencao',
              })
          }
        }
      }
    ...
    return err(
      `Ação desconhecida: "${action}". Ações disponíveis: listar_maquinas, criar_maquina, atualizar_maquina, deletar_maquina, registrar_horimetro, historico_horimetro, adicionar_despesa, listar_despesas, criar_manutencao, concluir_manutencao, listar_manutencao, criar_documento, listar_documentos, deletar_documento, resumo_frota, alertas`,
    )
  } catch (error: any) {
