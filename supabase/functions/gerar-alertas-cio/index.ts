  ...
  try {
    ...
    for (const animal of animais || []) {
      ...
      if (!existingAlerts || existingAlerts.length === 0) {
        const dataProximo = new Date(animal.proximo_cio_estimado).toLocaleDateString('pt-BR', {
          timeZone: 'UTC',
        })
        const mensagem = `Atenção: O animal ${animal.nome} (${animal.tipo}) está próximo de entrar em cio. Previsão estimada para ${dataProximo}.`

        const { error: insertError } = await supabase.from('alertas_cio').insert({
        ...
        if (resendApiKey && userEmail) {
          ...
          console.log(
            `[MOCK EMAIL] To: ${userEmail}, Subject: Alerta: ${animal.nome} entrando em cio\nBody: ${mensagem}`,
          )
          results.push({
            animal: animal.nome,
            email_sent: false,
            reason: 'No Resend API Key or missing email',
          })
        }
      }
    }
