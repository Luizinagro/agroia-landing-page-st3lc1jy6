...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Auth opcional — personaliza se logado, funciona sem login também
    let user: any = null;
    ...
    const { data: dentroDoLimite } = await supabase.rpc('check_rate_limit', {
      p_user_id: rateLimitKey, p_function: 'consultor-ia-agro', p_max_requests: 15, p_window_minutes: 10
    });
    if (dentroDoLimite === false) throw new Error('Limite de consultas atingido. Tente novamente em alguns minutos.');

    // Busca propriedade só se logado
    const propriedade = user ? await buscarPropriedade(supabase, user.id) : null;
...
5. Termine sempre com uma ação concreta para o produtor fazer HOJE`;
    ...
    if (!geminiRes.ok) {
      ...
      const errMsg  = errBody?.error?.message || errBody?.error?.status || `HTTP ${geminiRes.status}`;
      throw new Error(`Gemini erro: ${errMsg}`);
    }
    const geminiData = await geminiRes.json();
    const resposta = geminiData.candidates?.[0]?.content?.parts?.[0]?.text || '';
    if (!resposta) throw new Error('Gemini retornou resposta vazia. Tente novamente.');
