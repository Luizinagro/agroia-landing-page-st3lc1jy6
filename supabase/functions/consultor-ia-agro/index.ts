  ...
  "recomendacoes": "Recomendação detalhada e prática respondendo a pergunta do usuário considerando a região."
    ...
    if (!geminiResponse) {
      geminiResponse = {
        ...
        recomendacoes: `Considerando que você está em ${regiao} com clima ${regionalData.clima.toLowerCase()} e umidade de ${regionalData.umidade_solo}, recomendamos atenção ao planejamento de safra. Os preços variam em relação a outras regiões devido ao custo logístico local. (Resposta gerada via fallback pois a API Gemini falhou ou não está configurada)`
      };
    }
