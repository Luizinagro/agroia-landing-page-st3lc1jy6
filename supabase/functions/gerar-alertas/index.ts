  ...
  try {
    ...
    if (user_id) {
      const { data: props } = await supabase
        .from('propriedades')
        .select('nome, cultura_principal, latitude, longitude')
        .eq('user_id', user_id)
        .limit(1)
      ...
      alertas = [
        {
          tipo_alerta: 'clima',
          mensagem:
            'Alerta de chuvas isoladas previstas para as próximas horas na sua região cadastrada. Acompanhe a janela para adubação.',
          urgencia: 'alta',
          data_alerta: new Date().toISOString(),
          fonte: 'INMET',
        ...
        },
        {
          tipo_alerta: 'saude',
          mensagem:
            'Aviso de monitoramento contínuo devido às condições de umidade e temperatura reportadas perto da propriedade.',
          urgencia: 'baixa',
