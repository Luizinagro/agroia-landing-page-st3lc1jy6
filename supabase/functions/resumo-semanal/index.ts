...
async function buscarClimaPropriedade(lat: number, lon: number) {
  try {
    ...
    const chuva7d = (data.daily?.precipitation_sum?.slice(0, 7) ?? [])
...
async function buscarPrecosCultura(supabase: any, cultura: string) {
  try {
    ...
    const { data } = await supabase
      .from('precos_cache')
      .select('preco_saca, variacao_dia, unidade')
    ...
    for (const user of users || []) {
      try {
        ...
        const [
          ...
          supabase
            .from('propriedades')
            .select('nome, cultura_principal, area_hectares, latitude, longitude')
            .eq('user_id', user.id)
            .limit(3),
          supabase
            .from('satellite_analyses')
            .select('ndvi_value, soil_moisture, temperature, analysis_date')
            .eq('user_id', user.id)
            .order('analysis_date', { ascending: false })
            .limit(1),
          supabase.from('maquinas').select('nome').eq('user_id', user.id),
        ])
        ...
        const [climaData, precoData] = await Promise.all([
          prop?.latitude
            ? buscarClimaPropriedade(parseFloat(prop.latitude), parseFloat(prop.longitude))
            : null,
        ...
        const ndviStatus = analise?.ndvi_value
          ? analise.ndvi_value > 0.65
            ? '🟢 Saudável'
            : analise.ndvi_value > 0.45
              ? '🟡 Atenção'
              : '🔴 Crítico'
          : '⚪ Sem análise recente'
      ...
      <a href="https://agroia.goskip.app" style="color:#40916c;">agroia.goskip.app</a>
        ...
        if (resendKey && user.email) {
          ...
          results.push({
            email: user.email,
            success: false,
            motivo: resendKey ? 'email inválido' : 'RESEND_API_KEY não configurada',
          })
        }
      } catch (userErr) {
