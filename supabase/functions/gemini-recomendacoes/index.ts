...
Deno.serve(async (req) => {
  ...
  try {
          ...
          if (text) {
             try {
               ...
               if (jsonMatch) {
                 ...
                 recomendacoes = JSON.parse(text.replace(/```json/g, '').replace(/```/g, '').trim());
               }
             } catch(e) {
    ...
    if (!recomendacoes) {
      recomendacoes = {
        recomendacao_plantio: "Condições atuais favoráveis. Mantenha a janela de plantio planejada, garantindo boa profundidade (3-5cm) devido à umidade atual adequada.",
        recomendacao_insumo: "Atenção aos níveis de Nitrogênio (N). Recomendada aplicação de cobertura nos próximos 7 a 10 dias, aproveitando a previsão de chuva.",
        recomendacao_manejo: "Monitore áreas de baixada. Risco moderado de aparecimento de fungos foliares devido à combinação de alta umidade relativa e calor nos próximos dias.",
        urgencia: "média"
      };
    }
