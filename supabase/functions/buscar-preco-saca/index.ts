...
async function salvarCache(supabase: any, commodity: string, preco: number, unidade: string, variacao: number) {
  try {
    await supabase.from('precos_cache').insert({
      commodity, preco_saca: preco, unidade, variacao_dia: variacao, fonte: 'Gemini-IA'
    });
  } catch (e) { console.warn('Cache não salvo:', e); }
}

// Busca preços reais via Gemini com data atual injetada
async function buscarPrecoGemini(cultura: string, geminiKey: string): Promise<{ preco: number; variacao: number }> {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!cultura || quantidade === undefined) throw new Error('"cultura" e "quantidade" são obrigatórios.');

    const culturaKey = cultura.toLowerCase()
      .normalize('NFD').replace(/[̀-ͯ]/g, '') // remove acentos
      .replace(/\s+/g, '-');

    const culturasValidas = ['soja', 'milho', 'trigo', 'cafe', 'cafe-arabica', 'algodao', 'cana-de-acucar', 'boi-gordo', 'arroz'];
    if (!culturasValidas.includes(culturaKey)) throw new Error(`Cultura "${cultura}" não suportada.`);
    ...
    const { data: dentroDoLimite } = await supabase.rpc('check_rate_limit', {
      p_user_id: rateLimitKey, p_function: 'buscar-preco-saca', p_max_requests: 30, p_window_minutes: 10
    });
    ...
    if (cached) {
      ...
      await salvarCache(supabase, culturaKey, preco_saca, UNIDADES[culturaKey] || 'saca 60kg', variacao_dia);
    }
    ...
    return new Response(JSON.stringify({
      ...
      fonte,
      cache_age_horas,
      proxima_atualizacao_em_horas: cache_age_horas !== null ? Math.max(0, CACHE_HORAS - cache_age_horas) : 0,
