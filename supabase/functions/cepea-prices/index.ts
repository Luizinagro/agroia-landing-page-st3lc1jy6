...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const rateLimitKey = req.headers.get('Authorization') || req.headers.get('x-forwarded-for') || 'anon';
    const { data: dentroDoLimite } = await supabase.rpc('check_rate_limit', {
      p_user_id: rateLimitKey, p_function: 'cepea-prices', p_max_requests: 30, p_window_minutes: 10
    });
    if (dentroDoLimite === false) throw new Error('Limite de consultas atingido. Tente novamente em alguns minutos.');
    ...
    if (cached) {
      return new Response(JSON.stringify({
        ...
        fonte: 'cache',
        cache_age_min: Math.round((Date.now() - new Date(Object.values(cached)[0].created_at as string).getTime()) / 60000)
...
Use seus dados mais recentes. Seja realista e consistente com o mercado brasileiro atual.`;
    ...
    if (res.ok) {
      ...
      if (match) {
        ...
        // Salva no cache
        const cacheRows = [
          { commodity: 'boi-gordo', preco_saca: parsed['Boi Gordo']?.atual, unidade: '@15kg', variacao_dia: parsed['Boi Gordo']?.variacao || 0, fonte: 'Gemini-IA' },
          { commodity: 'bezerro', preco_saca: parsed['Bezerro']?.atual, unidade: 'cabeça', variacao_dia: parsed['Bezerro']?.variacao || 0, fonte: 'Gemini-IA' },
          { commodity: 'soja', preco_saca: parsed['Soja']?.atual, unidade: 'saca 60kg', variacao_dia: parsed['Soja']?.variacao || 0, fonte: 'Gemini-IA' },
          { commodity: 'milho', preco_saca: parsed['Milho']?.atual, unidade: 'saca 60kg', variacao_dia: parsed['Milho']?.variacao || 0, fonte: 'Gemini-IA' },
          { commodity: 'cafe-arabica', preco_saca: parsed['Café Arábica']?.atual, unidade: 'saca 60kg', variacao_dia: parsed['Café Arábica']?.variacao || 0, fonte: 'Gemini-IA' },
          { commodity: 'algodao', preco_saca: parsed['Algodão']?.atual, unidade: '@15kg', variacao_dia: parsed['Algodão']?.variacao || 0, fonte: 'Gemini-IA' }
...
function formatarPrecos(cached: Record<string, any>) {
  const map: Record<string, string> = {
    'boi-gordo': 'Boi Gordo', 'bezerro': 'Bezerro', 'soja': 'Soja',
    'milho': 'Milho', 'cafe-arabica': 'Café Arábica', 'algodao': 'Algodão'
  };
