...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === 'criar_funcionario') {
      ...
      const { data, error } = await admin.from('rh_funcionarios').insert({
        user_id: user.id, nome, cpf, funcao,
        data_admissao: data_admissao || new Date().toISOString().split('T')[0],
        salario_base: Number(salario_base)||0, telefone, observacao, ativo: true
      }).select().single();
    ...
    if (action === 'atualizar_funcionario') {
      ...
      const { data, error } = await admin.from('rh_funcionarios').update(campos).eq('id', id).eq('user_id', user.id).select().single();
      if (error) throw error;
      return ok(data);
    }
    ...
    if (action === 'desligar_funcionario') {
      ...
      if (!id) return err('"id" é obrigatório');
      const { data, error } = await admin.from('rh_funcionarios').update({
        ativo: false,
    ...
    if (action === 'registrar_ponto') {
      ...
      const { data: ponto, error } = await admin.from('rh_ponto').upsert({
        funcionario_id, data, hora_entrada, hora_saida,
        horas_trabalhadas: Number(horas_trabalhadas.toFixed(2)),
        horas_extras: Number(horas_extras)||0, observacao
    ...
    if (action === 'listar_ponto') {
      ...
      const { data, error } = await admin.from('rh_ponto')
        .select('*').eq('funcionario_id', funcionario_id)
        .gte('data', `${anoUsar}-${mesStr}-01`).lte('data', `${anoUsar}-${mesStr}-31`)
        .order('data');
      if (error) throw error;

      const total_horas  = (data||[]).reduce((s:number,p:any) => s + Number(p.horas_trabalhadas||0), 0);
      const dias_trabalhados = (data||[]).filter((p:any) => Number(p.horas_trabalhadas||0)>0).length;
      const horas_extras = (data||[]).reduce((s:number,p:any) => s + Number(p.horas_extras||0), 0);
      return ok({ pontos: data, resumo: { total_horas: Number(total_horas.toFixed(1)), dias_trabalhados, horas_extras: Number(horas_extras.toFixed(1)) } });
    }

    // ── ATIVIDADES POR FUNCIONÁRIO ────────────────────────────────

    if (action === 'registrar_atividade') {
      const { funcionario_id, data, descricao, talhao, cultura, horas, equipamento, observacao } = body;
      if (!funcionario_id || !descricao) return err('"funcionario_id" e "descricao" são obrigatórios');
      const { data: atv, error } = await admin.from('rh_atividades').insert({
        funcionario_id, data: data || new Date().toISOString().split('T')[0],
        descricao, talhao, cultura, horas: Number(horas)||0, equipamento, observacao
      }).select().single();
      if (error) throw error;
      return ok(atv);
    }
    ...
    if (action === 'listar_atividades') {
      ...
      const { data, error } = await q.order('data', { ascending: false }).limit(Number(limit)||100);
      if (error) throw error;
      return ok(data);
    }
    ...
    if (action === 'relatorio_mensal') {
      ...
      const { data: funcionarios } = await admin.from('rh_funcionarios').select('id, nome, funcao, salario_base').eq('user_id', user.id).eq('ativo', true);
      const relatorio = [];

      for (const f of (funcionarios||[])) {
        const { data: pontos } = await admin.from('rh_ponto').select('horas_trabalhadas, horas_extras, data')
          .eq('funcionario_id', f.id).gte('data', `${anoAtual}-${mesStr}-01`).lte('data', `${anoAtual}-${mesStr}-31`);
        const { data: atividades } = await admin.from('rh_atividades').select('descricao, horas, data')
          .eq('funcionario_id', f.id).gte('data', `${anoAtual}-${mesStr}-01`).lte('data', `${anoAtual}-${mesStr}-31`);

        const total_horas = (pontos||[]).reduce((s:number,p:any)=>s+Number(p.horas_trabalhadas||0),0);
        const horas_extras = (pontos||[]).reduce((s:number,p:any)=>s+Number(p.horas_extras||0),0);
      ...
      return ok({ mes: mesAtual, ano: anoAtual, funcionarios: relatorio, total_folha: Number(folha_total.toFixed(2)), total_funcionarios: relatorio.length });
    }
    ...
    if (action === 'dashboard_rh') {
      ...
      const { data: funcionarios } = await admin.from('rh_funcionarios').select('id, nome, funcao').eq('user_id', user.id).eq('ativo', true);
      const { data: atv_hoje } = await admin.from('rh_atividades').select('funcionario_id, descricao')
        .eq('data', hoje.toISOString().split('T')[0]);
      const { data: pontos_mes } = await admin.from('rh_ponto').select('funcionario_id, horas_trabalhadas')
        .gte('data', `${anoAtual}-${mesStr}-01`).lte('data', `${anoAtual}-${mesStr}-31`);

      const total_horas_mes = (pontos_mes||[]).reduce((s:number,p:any)=>s+Number(p.horas_trabalhadas||0),0);
    ...
    return err(`Ação desconhecida: "${action}". Disponíveis: criar_funcionario, listar_funcionarios, atualizar_funcionario, desligar_funcionario, registrar_ponto, listar_ponto, registrar_atividade, listar_atividades, relatorio_mensal, dashboard_rh`);
  } catch (error: any) {
